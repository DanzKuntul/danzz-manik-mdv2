...
let who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? Manik.user.jid : m.sender
let username = Manik.getName(who)
let logo = await (await fetch(`${logos()}`)).buffer()
let tag = `@${m.sender.split('@')[0]}`
 m, { contextInfo: { mentionedJid: Manik.parseMention(tag) }}
    let waofc = `@${'0'.split('@')[0]}`
 m, { contextInfo: { mentionedJid: Manik.parseMention(tag) }}
 ...
 m, { contextInfo: { mentionedJid: Manik.parseMention(tag) }}
    ...
    let tks = `Saya adalah Yanz BOT salah satu Bot Whatsapp yang diciptakan oleh : ${ow}
Silahkan tekan Tombol ${user.registered ? '"Menu"' : '"Verify"'} untuk ${user.registered ? 'melihat list command dari' : 'mendaftar sebelum menggunakan'} Yanz BOT.
...
function logos() {
  ...
  if (lgs >= 4) {
    imp = 'https://upload-xfar05.herokuapp.com/file/oKfah5Es4l22.jpg'
  }
  if (lgs > 10) {
    imp = 'https://upload-xfar05.herokuapp.com/file/oKfah5Es4l22.jpg'
  }
